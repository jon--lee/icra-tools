...
def collect_robust_traj_multiple(env, agent, ocs, T, opt, visualize=False, early_stop=True, clipped = False):
    ...
    for t in range(T):
        ...
        if score < .1 and t < (T - 1):
            ...
            a = a_intended
            print "Differences"
            rand_control = np.random.normal(0, .05, env.action_space.shape)
            s_alt, _, _, _ = env.step(rand_control)
            score_alt = ocs[t].decision_function([s])[0, 0]
            if score_alt < score:
                a = - alpha * rand_control
            else:
                a = alpha * rand_control
            print "Done computing differences\n\n"
